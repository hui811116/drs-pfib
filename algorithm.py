...
def supportedIBAlg():
	return ['admm1','admm2','logadmm1','logadmm2','admm1mask','admm2mask']
def getIBAlgorithm(method):
	...
	elif method == "admm1mask":
		return drsIBType1Mask
	elif method == "admm2mask":
		return drsIBType2Mask
	else:
		sys.exit("undefined method {:}".format(method))

def supportedPfAlg():
	return ['admm','admmarm','logadmm','admmmask']
...
def drsPFMask(pxy,nz,beta,convthres,maxiter,**kwargs):
	penalty = kwargs['penalty']
	alpha = kwargs['relax']
	ss_init = kwargs['sinit']
	ss_scale = kwargs['sscale']
	...
	# masks
	mask_pzcx = np.ones(pzcx.shape)
	mask_pzcy = np.ones(pzcy.shape)
	itcnt =0
	record_mat = np.zeros((1))
	if record_flag:
		record_mat = np.zeros((maxiter))
	...
	while itcnt < maxiter:
		...
		grad_y= gobj_pzcy(pzcy,pzcx,dual_y)
		mean_grad_y = grad_y - np.mean(grad_y,axis=0)
		ss_y = gd.validStepSize(pzcy,-mean_grad_y * mask_pzcy,ss_init,ss_scale)
		if ss_y ==0:
			if np.any(pzcy -mean_grad_y * mask_pzcy * 1e-9 >= 1.0):
				bad_cols = np.any(pzcy-mean_grad_y*1e-9>=1.0,axis=0)
				mask_pzcy[:,bad_cols] = 0
			else:
				break
		arm_y = gd.armijoStepSize(pzcy,-mean_grad_y*mask_pzcy,ss_y,ss_scale,1e-4,fobj_pzcy,gobj_pzcy,**{"pzcx":pzcx,"dual_y":dual_y})
		if arm_y == 0:
			arm_y = ss_y
		new_pzcy = pzcy - mean_grad_y*arm_y * mask_pzcy
		...
		grad_x = gobj_pzcx(pzcx,new_pzcy,dual_drs_y)
		mean_grad_x = grad_x-np.mean(grad_x,axis=0)
		ss_x = gd.validStepSize(pzcx,-mean_grad_x*mask_pzcx,ss_init,ss_scale)
		if ss_x == 0:
			if np.any(pzcx-mean_grad_x * mask_pzcx * 1e-9 >= 1.0):
				bad_cols = np.any(pzcx - mean_grad_x * 1e-9>=1.0,axis=0)
				mask_pzcx[:,bad_cols] = 0
			else:
				break
		arm_x = gd.armijoStepSize(pzcx,-mean_grad_x*mask_pzcx,ss_x,ss_scale,1e-4,fobj_pzcx,gobj_pzcx,**{"pzcy":new_pzcy,"dual_y":dual_drs_y})
		if arm_x == 0:
			arm_x = ss_x
		new_pzcx = pzcx - ss_x * mean_grad_x * mask_pzcx
		new_pz = np.sum(new_pzcx * px[None,:],axis=1)
...
def drsIBType1Mask(pxy,nz,beta,convthres,maxiter,**kwargs):
	penalty = kwargs['penalty']
	alpha = kwargs['relax']
	gamma = 1/ beta
	ss_init = kwargs['sinit']
	...
	# masking the update for better convergence... and performance
	mask_pzcx = np.ones((nz,nx))
	itcnt =0
	record_mat = np.zeros((1))
	if record_flag:
		record_mat = np.zeros((maxiter))
	...
	while itcnt < maxiter:
		...
		ss_x = gd.validStepSize(pzcx,-mean_grad_x * mask_pzcx,ss_init,ss_scale)
		if ss_x == 0:
			bad_cols = np.any(pzcx - 1e-9*mean_grad_x>=1.0,axis=0)
			mask_pzcx[:,bad_cols] = 0
			#break
		arm_x = gd.armijoStepSize(pzcx,-mean_grad_x * mask_pzcx,ss_x,ss_scale,1e-4,fobj_pzcx,gobj_pzcx,**{"pz":new_pz,"dual_z":dual_z})
		if arm_x == 0:
			arm_x = ss_x
		new_pzcx = pzcx - arm_x * mean_grad_x * mask_pzcx
		errz = new_pz - np.sum(new_pzcx*px[None,:],axis=1)
		dtvz = 0.5* np.sum(np.fabs(errz),axis=0)
		if np.all(np.array(dtvz<convthres)):
			conv = True
...
def drsIBType2Mask(pxy,nz,beta,convthres,maxiter,**kwargs):
	penalty = kwargs['penalty']
	alpha = kwargs['relax']
	gamma = 1/ beta
	ss_init = kwargs['sinit']
	...
	while itcnt < maxiter:
		...
		#		+(dual_zy + penalty * err_zy)@pxcy.T
		mean_gx = grad_x - np.mean(grad_x,0)
		ss_x = gd.validStepSize(pzcx,-mean_gx * mask_pzcx,ss_init,ss_scale)
		if ss_x ==0:
			if np.any(pzcx - mean_gx*mask_pzcx * 1e-9 >= 1.0):
				bad_cols = np.any(pzcx - mean_gx * 1e-9 >=1.0,axis=0)
				mask_pzcx[:,bad_cols] = 0
			else:
				break
		arm_x = gd.armijoStepSize(pzcx,-mean_gx * mask_pzcx,ss_x,ss_scale,1e-4,fobj_pzcx,gobj_pzcx,**{"pz":pz,"pzcy":pzcy,"dual_z":dual_z,"dual_zcy":dual_zy})
		if arm_x == 0:
			arm_x = ss_x
		new_pzcx = pzcx - mean_gx * ss_x * mask_pzcx
		# relaxation step
		err_z = np.sum(new_pzcx * px[None,:],axis=1) - pz
		err_zy = new_pzcx@pxcy - pzcy
		relax_z = dual_z - (1-alpha)*penalty*err_z
		...
		mean_gy = grad_y - np.mean(grad_y,0)
		# joint stepsize selection
		ss_z = gd.validStepSize(pz,-mean_gz * mask_pz,ss_init,ss_scale)
		if ss_z ==0:
			if np.any(pz-mean_gz * mask_pz * 1e-9>=1.0):
				mask_pz= np.zeros(pz.shape)
			else:
				break
		arm_z= gd.armijoStepSize(pz,-mean_gz*mask_pz,ss_z,ss_scale,1e-4,fobj_pz,gobj_pz,**{"pzcx":new_pzcx,"dual_z":relax_z})
		if arm_z == 0:
			arm_z = ss_z
		ss_y = gd.validStepSize(pzcy,-mean_gy*mask_pzcy,arm_z,ss_scale)
		if ss_y ==0:
			if np.any(pzcy- mean_gy*1e-9*mask_pzcy>=0):
				bad_cols = np.any(pzcy- mean_gy*1e-9>=0,axis=0)
				mask_pzcy[:,bad_cols] = 0
			else:
				break
		arm_y = gd.armijoStepSize(pzcy,-mean_gy*mask_pzcy,ss_y,ss_scale,1e-4,fobj_pzcy,gobj_pzcy,**{"pzcx":new_pzcx,"dual_zcy":relax_zy})
		if arm_y == 0:
			arm_y = ss_y
		new_pz = pz - mean_gz * arm_y * mask_pz
		new_pzcy = pzcy - mean_gy * arm_y * mask_pzcy
		# dual update
		err_z = np.sum(new_pzcx * px[None,:],axis=1) - new_pz
		err_zy = new_pzcx@ pxcy - new_pzcy
...
def admmIBLogSpaceType1(pxy,nz,beta,convthres,maxiter,**kwargs):
	...
	while itcnt < maxiter:
		...
		grad_x = -gamma * mexp_mlog_pzcx* (1-mlog_pzcx)*px[None,:] + mexp_mlog_pzcx*((1+np.log(tmp_pzcy))@pxy.T) \
				- (mexp_mlog_pzcx * px[None,:])*((dual_z + penalty * err_z)/est_pz)[:,None]
